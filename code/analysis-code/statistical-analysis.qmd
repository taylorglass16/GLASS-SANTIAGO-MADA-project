...
rsq1 <- rsq(popdata, truth = sarc_standard, estimate = .pred)

# create predictions using the pct_currentlymarried predictor model
...
rsq2 <- rsq(marrieddata, truth = pct_currentlymarried, estimate = .pred)

# create predictions using the rate of maternal deaths predictor model
matdeathspred <- predict(lmfit5, new_data = mydata) %>% 
                  select(.pred)

# create a data frame with the predictions and true values 
matdeathdata <- bind_cols(matdeathspred, mydata$rate_matdeaths) %>%
            rename(rate_matdeaths = "...2")

# find RMSE and R squared to determine model fit
rmse3 <- rmse(matdeathdata, truth = rate_matdeaths, estimate = .pred)
rsq3 <- rsq(matdeathdata, truth = rate_matdeaths, estimate = .pred)
