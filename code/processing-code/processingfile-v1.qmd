...
rawdata2 <- rawdata %>% 
                select(country, region, subregion, pct_upreg, upreg_subreg, pct_upreg_larcster, pct_upreg_sarc, pct_upreg_trad, pct_upreg_nouse, larc_1519, sarc_1519, tradmethods_1519, nomethod_1519, larc_2024, sarc_2024, tradmethods_2024, nomethod_2024, larc_2534,  sarc_2534, tradmethods_2534,  nomethod_2534, larc_3549, sarc_3549, tradmethods_3549, nomethod_3549)
...
We should impute the missing data here to avoid introducing bias because 18.18% of the data is missing, and this variable is essential for answering the research question. After finding the summary statistics of this variable, the median is 0.4701 and the mean is 0.4679, which suggests that there are minimal outliers in the data.  
```{r}
#| message: false
...
# ensure mutation worked correctly
summary(rawdata2$pct_upreg)
```
...
rawdata2 %>% skim(total_larc, total_sarc, total_trad, total_none)
```
...
rawdata2 <- rawdata2 %>% 
              mutate(larc_usage = as.factor(case_when(
                                  total_larc < 11.5 ~ "Low",
                                  total_larc >= 11.5 & total_larc < 407.4633 ~ "Medium",
                                  total_larc >= 407.4633 ~ "High")))

# check if new variable was created correctly 
class(rawdata2$larc_usage)

# create usage variable for short acting reversible methods 
rawdata2 <- rawdata2 %>% 
              mutate(sarc_usage = as.factor(case_when(
                                  total_sarc < 114.33113 ~ "Low",
                                  total_sarc >= 114.33113 & total_larc < 1554.7609 ~ "Medium",
...
rawdata2 <- rawdata2 %>% 
              mutate(trad_usage = as.factor(case_when(
                                  ...
                                  total_trad >= 15.05000 & total_larc < 332.4750 ~ "Medium",
...
rawdata2 <- rawdata2 %>% 
              mutate(none_usage = as.factor(case_when(
                                  ...
                                  total_none >= 80.07500 & total_larc < 875.2750 ~ "Medium",
