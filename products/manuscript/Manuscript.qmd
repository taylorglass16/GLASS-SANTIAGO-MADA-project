...
Is there a significant difference in the percentage of unintended pregnancies among short acting reversible contraception users among region? 
...
#| message: false
#| label: fig-1
#| fig-cap: "[insert explanation here]"
#| echo: FALSE
...
This study relied on secondary data from the Guttmacher Institute's Adding It Up project. This means that we were limited by the data that was available and unable to collect our own data. Also, the study was cross-sectional, which means that it only looked at data from one point in time. This makes it difficult to establish causal relationships between the variables.

Despite these limitations, our study provides valuable insights into the factors influencing unintended pregnancy rates in low- and middle-income countries. The findings suggest that a multifaceted approach is needed to address unintended pregnancy. This approach should include increasing access to SARC and LARC methods, improving education and counseling on contraception use, and addressing regional disparities in access to healthcare services and family planning programs.
