...
# Research Questions
1) What is the relationship between rates of unintended pregnancies and usage of contraceptive methods across various age groups?
The main outcome variables will be total percentage of unintended pregnancies (`pct_upreg`). 
The predictor variables will include percentage of unintended pregnancies across three types of birth control: long acting reversible methods (`pct_upreg_larcster`), short term reversible methods (`pct_upreg_sarc`), traditional methods (`pct_upreg_trad`), and no method (`pct_upreg_nouse`). No method variables can serve as control groups for statistical tests.
...
2) How does marriage status affect percentage of unintended pregnancies across different types of birth control?
  -outcome variables:
      unintended pregnancies, total number (pct_upreg)
      unintended pregnancies, using larc and sterilization (pct_upreg_larcster)
      unintended pregnancies, using short term reversible (pct_upreg_sarc)
  -predictor variables:
      Number wanting to avoid pregnancy, all married women (inneed_married)
      Number using sterilization, all married women	(ster_married) **combine these two in processing
      Number using long-acting reversible methods, all married women	(larc_married)
      Number using short-acting methods, all married women	(sarc_married)
      Number wanting to avoid pregnancy, formerly married women 	(inneed_formerlymar)
      Number using sterilization, formerly married women	(ster_formerlymar)
      Number using long-acting reversible methods, formerly married women	(larc_formerlymar)
      Number using short-acting methods, formerly married women	(sarc_formerlymar)
      Number wanting to avoid pregnancy, never married women 	(inneed_nevermar)
      Number using all modern methods, never married women	(modmethods_nevermar)
      Number using sterilization, never married women	(ster_nevermar)
      Number using long-acting reversible methods, never married women	(larc_nevermar)
      Number using short-acting methods, never married women	(sarc_nevermar)
