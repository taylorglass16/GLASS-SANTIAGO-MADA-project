...
pred_summary <- mydata %>%
  summarise(across(c(wra, pct_currentlymarried,pct_nevermarried,rate_matdeaths, rate_matdeaths_upreg, rate_matdeaths_abs), mean, sd))

# Descriptive statistics for categorical variable (assuming region is a factor)
summary_factor <- mydata %>%
  group_by(region) %>%
  summarise(n = n())

# Combine into a single dataframe
summary_pred <- rbind(
  pred_summary %>% summarise(Variable = "pct_upreg_sarc", ., .names = quote(names(.))),
  pred_summary %>% select(-pct_upreg_sarc),
  summary_factor %>% summarise(Variable = "region", n)
)

# Create table 
kable(summary_pred, caption = "Table Depicting Predictor Variables", booktabs = TRUE) %>%
  kable_styling(full_width = F, font_size = 10) %>%
  column_def(widths = c(200, 150, 150))
